(function (root) {
  ...
  function tableXml(block, widthPt, heightPt) {
    ...
    const body = Array.from({ length: rows }, (_, row) => { const rowH = rowHeights[row]; return `<w:tr><w:trPr><w:trHeight w:val="${rowH}" w:hRule="exact"/></w:trPr>${Array.from({ length: cols }, (_, col) => { const cellW = columnWidths[col], index = row * cols + col, cellText = cells[index] || "", image = block.cellImages?.[index], rid = block._cellImageRids?.[index], maxW = Math.max(18, cellW / 20 - 10), textReserve = String(cellText).trim() ? 26 : 0, maxH = Math.max(18, rowH / 20 - 12 - textReserve), ratio = Number(image?.ratio) || 1, imageW = Math.min(maxW, maxH * ratio), imageH = imageW / ratio, picture = rid ? `<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:before="0" w:after="0"/></w:pPr><w:r><w:pict><v:shape type="#_x0000_t75" style="width:${pt(imageW)}pt;height:${pt(imageH)}pt" stroked="f"><v:imagedata r:id="${rid}" o:title="${xml(image?.name || "儲存格圖片")}"/></v:shape></w:pict></w:r></w:p>` : ""; return `<w:tc><w:tcPr><w:tcW w:w="${cellW}" w:type="dxa"/><w:vAlign w:val="center"/><w:tcMar><w:top w:w="50" w:type="dxa"/><w:left w:w="70" w:type="dxa"/><w:bottom w:w="50" w:type="dxa"/><w:right w:w="70" w:type="dxa"/></w:tcMar></w:tcPr>${picture}${textParagraphs(block, cellText)}</w:tc>`; }).join("")}</w:tr>`; }).join("");
  ...
  function shapeXml(block, index, page) {
    const metrics = pageMetrics(page), left = block.x / metrics.width * metrics.ptW, top = block.y / metrics.height * metrics.ptH, width = block.w / metrics.width * metrics.ptW, height = block.h / metrics.height * metrics.ptH, position = `position:absolute;margin-left:${pt(left)}pt;margin-top:${pt(top)}pt;width:${pt(width)}pt;height:${pt(height)}pt;z-index:${251659264 + index};mso-position-horizontal-relative:page;mso-position-vertical-relative:page`;
    if (block.type === "image") return `<w:p><w:pPr><w:spacing w:before="0" w:after="0" w:line="1" w:lineRule="exact"/></w:pPr><w:r><w:pict><v:shape id="PaperGridImage${index + 1}" type="#_x0000_t75" style="${position}" o:allowincell="f" filled="f" stroked="f"><v:wrap type="none"/><v:imagedata r:id="${block._imageRid}" o:title="${xml(block.name || "圖片")}"/></v:shape></w:pict></w:r></w:p>`;
  ...
  function buildDocx(blocks, title, page = DEFAULT_PAGE) {
    ...
    const files = {
      ...
      "word/_rels/document.xml.rels": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${prepared.relationships.join("")}</Relationships>`,
